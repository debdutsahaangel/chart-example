...
class LineChartCircleCenterRenderer(
     ...
     */
    private val mCirclePaintInner: Paint by lazy { Paint(Paint.ANTI_ALIAS_FLAG) }
    ...
    private fun drawLinear(c: Canvas?, dataSet: ILineDataSet) {
        ...
        if (dataSet.colors.size > 1) {
            ...
            for (j in mXBounds.min..mXBounds.range + mXBounds.min) {
                ...
                mLineBuffer[1] = e?.y?.times(phaseY) ?: 0f
                if (j < mXBounds.max) {
            ...
            if (e1 != null) {
                var j = 0
                for (x in mXBounds.min until mXBounds.range.plus(mXBounds.min).plus(1)) {
                    e1 = dataSet.getEntryForIndex(if (x == 0) 0 else x - 1)
                    e2 = dataSet.getEntryForIndex(x)
                    if (e1 == null || e2 == null) continue
                    mLineBuffer[j++] = e1.x.plus(0.5f)
                    mLineBuffer[j++] = e1.y * phaseY
                    if (isDrawSteppedEnabled) {
                        mLineBuffer[j++] = e2.x
                        mLineBuffer[j++] = e1.y * phaseY
                        mLineBuffer[j++] = e2.x * 1.5f
                        mLineBuffer[j++] = e1.y * phaseY
                    }
                    mLineBuffer[j++] = e2.x.plus(0.5f)
    ...
    private fun drawCircles(c: Canvas) {
        ...
        for (i in dataSets.indices) {
            ...
            for (j in mXBounds.min..boundsRangeCount) {
                ...
                mCirclesBuffer[0] = e.x.plus(0.5f)
